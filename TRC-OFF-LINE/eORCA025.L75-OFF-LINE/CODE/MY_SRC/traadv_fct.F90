...
CONTAINS
   ...
   SUBROUTINE tra_adv_fct( kt, kit000, cdtype, p2dt, pun, pvn, pwn,       &
      ...
      IF( MOD( kt - 1, 10 ) == 0 )   THEN
         IF(lwp) WRITE(numout,*) 'MIN & MAX un', MINVAL(pun) , MAXVAL(pun)
         IF(lwp) WRITE(numout,*) 'MIN & MAX vn', MINVAL(pvn) , MAXVAL(pvn)
         IF(lwp) WRITE(numout,*) 'MIN & MAX ptn', MINVAL(ptn(:,:,:,1)) , MAXVAL(ptn(:,:,:,1))
         IF(lwp) WRITE(numout,*) 'MIN & MAX ptb', MINVAL(ptb(:,:,:,1)) , MAXVAL(ptb(:,:,:,1))
      ENDIF
      !
