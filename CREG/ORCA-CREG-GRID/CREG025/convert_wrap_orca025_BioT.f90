...
double precision, pointer, dimension(:) :: z1d
...
integer nargc
integer :: natts, attid
character(len=255) :: clname, cdname, varnameid
...
!----------------------------------------------------------
      ...
      status=NF90_INQ_VARID         (ncid, varnameid, varid)
      if (status/=0) then
          write(*,*) 'variable ',varname,' not found'
          stop
      endif
...
!----------------------------------------------------------
      ...
      status=NF90_DEF_DIM( ncout, 'deptht', nz, dimz )
...
! jump over undesired variables
    if ( varname == 'x' .or. varname == 'y' .or. varname == 'e3t' .or. varname == 'time_instant' .or. varname == 'time_instant_bounds' .or. & 
         varname == 'time_counter' .or. varname == 'time_centered' .or. varname == 'sossheig' .or. varname == 'deptht_bounds' .or. & 
         varname == 'axis_nbounds' .or. varname == 'nav_lev') cycle

    IF ( varname == 'nav_lat' .or. varname == 'nav_lon' .or. varname == 'vosaline' .or. varname == 'votemper' .or. varname == varnameid .or. varname == 'deptht' )  then
        if ( ndims == 1 ) then ; ddims(1)=dimz ; endif
        ...
        if ( varname == 'deptht' ) then 
    		istart(:)=1; icount(:)=1;
    		istart(1)=1
    		icount(1)=nz
    		status=NF90_GET_VAR(ncid, varid, z1d, istart, icount)
    		status=NF90_PUT_VAR( ncout, varout, z1d, istart, icount)
