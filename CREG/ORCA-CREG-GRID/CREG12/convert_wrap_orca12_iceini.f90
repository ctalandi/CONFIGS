...
integer :: ista=161, jsta=1813, jdiff=0  ! ista: distance from middle (x-direction)
                 ...
                 ! else set jdiff=0 
integer status
integer ncid, varid, ncout, dimx, dimy, dimt, ndims, nvars, itype, ipos, varout
...
!----------------------------------------------------------
      ...
      status=NF90_INQ_VARID         (ncid, "iicethic", varid)
      if (status/=0) then
          write(*,*) 'variable iicethic not found'
          stop
      endif
...
!----------------------------------------------------------
      ...
      status=NF90_DEF_DIM( ncout, 'time_counter', nt, dimt )
      first=.false.
...
! jump over undesired variables
    if ( varname == 'iiceprod'     .or. varname == 'ioceflxb' .or. varname == 'iicevelu' .or.  &
         varname == 'time_counter' .or. varname == 'iicevelv' .or. varname == 'iicestru' .or.  &
         varname == 'iicestrv'     .or. varname == 'iicesflx' .or. varname == 'iicenflx' .or.  &
         varname == 'isnowpre' ) cycle
