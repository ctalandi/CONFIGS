...
902   IF( ios >  0 )   CALL ctl_nam ( ios , 'namdyn_ldf in configuration namelist' )
   ...
   SUBROUTINE ldf_dyn( kt, Kbb )
      ...
      !!                         ( |u|e /2  or  |u|e^3/12 for laplacian or bilaplacian operator )
      !!
         ...
         IF( ln_dynldf_lap   ) THEN        ! laplacian operator : |u| e /2 = |u/4| e
            DO jk = 1, jpkm1
               DO_2D( 0, 0, 0, 0 )
                  ...
                  ahmt(ji,jj,jk) = SQRT( (zu2pv2_ij + zu2pv2_ij_m1) * r1_8 ) * zemax * tmask(ji,jj,jk)      ! 8= 2*2 * 2
                  zemax = MAX( e1f(ji,jj) , e2f(ji,jj) )
                  ahmf(ji,jj,jk) = SQRT( (zu2pv2_ij + zu2pv2_ij_p1) * r1_8 ) * zemax * fmask(ji,jj,jk)      ! 8= 2*2 * 2
               END_2D
            END DO
