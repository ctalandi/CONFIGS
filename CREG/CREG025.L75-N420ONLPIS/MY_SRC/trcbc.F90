...
CONTAINS
   ...
   SUBROUTINE trc_bc_ini( ntrc, Kmm )
      ...
      !!
      NAMELIST/namtrc_bc/ cn_dir_obc, sn_trcobc, rn_trofac, cn_tronam, cn_dir_sbc, sn_trcsbc, rn_trsfac, & 
...
9003  FORMAT(a, i5, a)
      ...
      IF( ln_bdy .AND. nb_trcobc > 0 ) THEN 
         ...
         DO ib = 1, nb_bdy
            ...
            ! namtrc_bc from the namelist_cfg internal file.
            ! Do this by finding the jbdy'th occurence of namtrc_bc in the
            ! character buffer as the starting point.
            !
            nbdy_loc = INDEX( numnat_cfg( nbdy_rdstart: ), 'namtrc_bc' )
            IF( nbdy_loc .GT. 0 ) THEN
               nbdy_rdstart = nbdy_rdstart + nbdy_loc
            ELSE
               WRITE(cerrmsg,'(A,I4,A)') 'Error: entry number ',ib,' of namtrc_bc not found'
               ios = -1
               CALL ctl_nam ( ios , cerrmsg )
            ENDIF
            READ  ( numnat_cfg( MAX( 1, nbdy_rdstart - 2 ): ), namtrc_bc, IOSTAT = ios, ERR = 902)
902         IF( ios >  0 )   CALL ctl_nam ( ios , 'namtrc_bc in configuration namelist' )
            IF(lwm) WRITE( numond, namtrc_bc )
            ...
            DO jn = 1, ntrc
               ...
               IF( ln_trc_obc(jn) ) THEN     !* Initialise from external data *!
                  ...
                  ENDIF
                  trcdta_bdy(jl,ib)%trc => sf_trcobc(jl,ib)%fnow(:,1,:)
                  trcdta_bdy(jl,ib)%rn_fac = rf_trofac(jl,ib)
